...
namespace NoSqlDb
{
  ...
  public:
    PayLoad() = default;
    PayLoad(const std::string& val) : value_(val),isClose_(false) {}
    ...
    operator std::string() {
		...
		returnValue.append( value_.substr(0, 30));
		returnValue.append("    "+status_);
		for (auto cat:categories_)
			returnValue.append("    "+cat);
		return returnValue; 
	}
	...
	void isClose(const bool isClose) { isClose_ = isClose; }
  ...
  */
  ...
  // showElementPayLoad display functions
